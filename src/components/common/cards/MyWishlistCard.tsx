...
const MyWishlistCard = ({
  ...
  return (
    <article className="w-full rounded-[28px] border border-slate-200 bg-white p-10 shadow-sm">
      <div className="flex items-start justify-between">
        <span
          className={`inline-flex rounded-full px-5 py-3 text-xl font-semibold ${config.badgeClassName}`}
        >
          {config.label}
        </span>

        <span className={`text-4xl font-extrabold ${dayStatus.className}`}>
          {dayStatus.text}
        </span>
      </div>

      <h2 className="mt-10 text-4xl font-extrabold leading-tight text-slate-900">
        {title}
      </h2>
